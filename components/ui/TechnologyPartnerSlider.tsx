...
async function fetchBanners() {
  const response = await fetch('https://dev-api.instient.ai/api/technologypartnerbanners?populate=*', {
        ...
        {banners.map((banner, index) => (
          <SwiperSlide key={index} className="flex ">
            <div className="container mx-auto flex flex-col md:flex-row items-center  md:h-[400px]">
              <div className="md:w-1/2 mx-14 px-10"> {/* Adjusted padding */}
              ...
              <div className="md:w-1/2 flex justify-end h-[300px] md:h-[400px] mt-4 md:mt-0"> {/* Reduced height for mobile and added margin-top */}
                <Image
                  src={`https://dev-api.instient.ai${banner.banner_image?.url}`}
