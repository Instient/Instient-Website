...
export default function ApplyJobModal({ isOpen, onClose, jobTitle }: ApplyJobProps): JSX.Element {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full sm:w-[90%] md:w-[60%] max-w-xl py-5 font-ubuntu overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-3">
          <div className="flex flex-row space-x-4">
            <div className="w-1/2">
              ...
              <Input id="firstName" placeholder="First Name" required value={firstName} onChange={(e) => setFirstName(e.target.value)} />
            </div>

            <div className="w-1/2">
              <Label htmlFor="lastName">Last Name *</Label>
              <Input id="lastName" placeholder="Last Name" required value={lastName} onChange={(e) => setLastName(e.target.value)} />
            </div>
          </div>
          ...
          <div className="flex flex-row space-x-4">
            <div className="w-1/2">
              ...
              <div {...getResumeRootProps()} className="border-dashed border-2 p-2 text-center cursor-pointer">
                <input {...getResumeInputProps()} />
                {resume ? <p>{resume.name}</p> : <p className="text-xs">Drag & drop your resume here, or click to upload</p>}
              </div>
            </div>

            <div className="w-1/2">
              <Label>Cover Letter Upload</Label>
              <div {...getCoverLetterRootProps()} className="border-dashed border-2 p-2 text-center cursor-pointer">
                <input {...getCoverLetterInputProps()} />
                {coverLetter ? <p>{coverLetter.name}</p> : <p className="text-xs">Drag & drop your letter here, or click to upload</p>}
              </div>
            </div>
          </div>
