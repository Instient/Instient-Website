...
function renderContent(content) {
  ...
  return content.map((block, index) => {
    if (block.type === "paragraph") {
      return (
        <p key={index} className="text-xl px-3 sm:p-0 font-ubuntu">
      ...
      return (
        <ul
          ...
          className={`text-xl px-3 sm:p-0 font-ubuntu ${
            block.format === "unordered" ? "list-disc pl-6" : "list-decimal pl-6"
          }`}
        >
...
function Section({ contentTitle, contentAnswer }) {
  return (
