...
async function fetchAccessibilityPageData() {
  ...
  const response = await fetch(
    `https://dev-api.instient.com/api/accessibilitypage?populate=*`, 
    {
      headers: {
...
export default async function AccessibilityPage() {
  const accessibilityData = await fetchAccessibilityPageData();

  // If no data or malformed data, show an error message
  if (!accessibilityData || !accessibilityData.Title || !accessibilityData.header || !accessibilityData.Description) {
  ...
  const { Title, header, Description, content1, content1_answer, content2, content2_answer, conclusion, Image: { url } = {},   } = accessibilityData;
