...
async function fetchCookieSettingsPageData() {
  ...
  const response = await fetch(
    `https://dev-api.instient.com/api/cookiesettingspage?populate=*`, 
    {
      headers: {
...
export default async function CookieSettingsPage() {
  const cookieSettingsData = await fetchCookieSettingsPageData();

  // If no data or malformed data, show an error message
  if (!cookieSettingsData || !cookieSettingsData.Title || !cookieSettingsData.header || !cookieSettingsData.Description) {
  ...
  const { Title, header, Description, content1, content1_answer, content2, content2_answer, conclusion, Image: { url } = {},  } = cookieSettingsData;
