...
async function fetchWhyJoinPageData() {
  ...
  const response = await fetch(
    `https://dev-api.instient.com/api/whyjoininstient`, 
    {
      headers: {
...
export default async function WhyJoinInstientPage() {
  ...
  if (!whyJoinData || !whyJoinData.Title || !whyJoinData.header || !whyJoinData.Description) {
  ...
  const { Title, header, Description, content1, content1_answer, content2, content2_answer, conclusion } = whyJoinData;
