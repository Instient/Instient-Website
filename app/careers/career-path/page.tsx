...
export default function CareerPath() {
    ...
    useEffect(() => {
      const fetchCareerPathData = async () => {
        try {
          const response = await fetch("https://dev-api.instient.com/api/careerpathpage?populate=*", {
            headers: {
  ...
  return (
    <main>
      <div className="w-full h-[425px] sm:h-[450px] p-6 font-ubuntu relative">
        {/* Background Image */}
        <Image
          src={`https://dev-api.instient.com${careerPathData.Image.url}`} // Dynamically set the full image URL from the API
