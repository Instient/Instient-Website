...
async function fetchCaseStudyData(slug: string) {
  ...
  const response = await fetch(
    `https://dev-api.instient.com/api/casestudies?filters[slug][$eq]=${slug}`, 
    {
      headers: {
...
export default async function CaseStudySlugPage({ params }: { params: { slug: string } }) {
  ...
  const { Title, header, Description, content1, content1_answer, content2, content2_answer, conclusion } = caseStudyData;
