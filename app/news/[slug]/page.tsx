...
async function fetchNewsData(slug: string) {
  ...
  const response = await fetch(
    `http://localhost:1337/api/news-instients?filters[slug][$eq]=${slug}`, 
    {
      headers: {
...
export default async function NewsSlugPage({ params }: { params: { slug: string } }) {
  ...
  if (!newsData || !newsData.Title || !newsData.header || !newsData.Description) {
    return <p className="text-center mt-20">Some required fields are missing or the article does not exist.</p>;
  }

  // Destructure attributes safely
  const { Title, header, Description, content1, content1_answer, content2, content2_answer, conclusion } = newsData;
